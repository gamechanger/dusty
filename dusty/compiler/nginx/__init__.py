...
def _nginx_http_spec(port_spec):
...
def _nginx_stream_spec(port_spec):
...
def get_nginx_configuration_spec(port_spec_dict):
    ...
    for port_spec in port_spec_dict['nginx']:
        if port_spec['type'] == 'http':
            nginx_http_config += _nginx_http_spec(port_spec)
        elif port_spec['type'] == 'stream':
            nginx_stream_config += _nginx_stream_spec(port_spec)
