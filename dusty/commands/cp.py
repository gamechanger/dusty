...
def copy_from_local(local_path, remote_name, remote_path, demote=True):
    ...
    if os.path.isdir(local_path):
        sync_local_dir_to_vm(local_path, os.path.join(constants.VM_CP_DIR, remote_name, temp_identifier), demote=demote)
        move_dir_inside_container(remote_name, os.path.join(constants.CONTAINER_CP_DIR, temp_identifier), remote_path)
    else:
        sync_local_file_to_vm(local_path, os.path.join(constants.VM_CP_DIR, remote_name, temp_identifier), demote=demote)
